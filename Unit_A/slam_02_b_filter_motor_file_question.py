...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        old_center_x = old_pose[0] - scanner_displacement*cos(old_pose[2])
        old_center_y = old_pose[1] - scanner_displacement*sin(old_pose[2])

        R = (motor_ticks[0]*ticks_to_mm)/alpha
        cx = old_center_x - (R+robot_width/2)*sin(old_pose[2])
        cy = old_center_y + (R+robot_width/2)*cos(old_pose[2])
        theta = (old_pose[2] + alpha)%(2*pi)  # mod 2pi
        new_center_x = cx + (R+robot_width/2)*sin(theta)
        new_center_y = cy - (R+robot_width/2)*cos(theta)
        
        x = new_center_x + scanner_displacement*cos(theta)
        y = new_center_y + scanner_displacement*sin(theta)
