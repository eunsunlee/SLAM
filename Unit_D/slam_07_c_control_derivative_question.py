...
#            pass  # Remove this.
            ...
            dg1_l = wr * (sin(theta+alpha)-sin(theta)) - r2l * cos(theta+alpha)
            dg2_l = wr * (-cos(theta+alpha)+cos(theta)) - r2l * sin(theta+alpha)
            dg3_l = - (1/w)
        
            dg1_r = -wl * (sin(theta+alpha)-sin(theta)) + r2l * cos(theta+alpha)
            dg2_r = -wl * (-cos(theta+alpha)+cos(theta)) + r2l * sin(theta+alpha)
            dg3_r = 1 / w
            
            m = array([[dg1_l, dg1_r], [dg2_l, dg2_r], [dg3_l, dg3_r]])
...
#            pass  # Remove this.
            dg1_l = .5 * (cos(theta) + (l/w)*sin(theta))
            dg2_l = .5 * (sin(theta) - (l/w)*cos(theta))
            dg3_l = - 1/w
        
            dg1_r = .5 * ((-l/w)*sin(theta) + cos(theta))
            dg2_r = .5 * ((l/w)*cos(theta) + sin(theta))
            dg3_r = 1 / w
